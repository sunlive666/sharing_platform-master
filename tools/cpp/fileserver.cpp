...
int main(int argc,char *argv[])
{
    if (argc!=3)
    {
      ...
      printf("Example:./fileserver 5005 /myproject/log/idc/fileserver.log\n"); 
      printf("         /myproject/tools/bin/procctl 10 /myproject/tools/bin/fileserver 5005 /myproject/log/idc/fileserver.log\n\n\n"); 
      return -1;
    }
...
void recvfilesmain()
{
    while (true)
    {
        ...
        {
            // 解析上传文件请求报文的xml。
            string clientfilename;   // 对端的文件名绝对路径。
            string mtime;               // 文件的时间。
            int  filesize=0;              // 文件大小。
...
void sendfilesmain()
{
    pactive.addpinfo(starg.timeout,starg.pname); //感觉此处没必要再加进程信息
