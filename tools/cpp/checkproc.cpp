/****************************************************************************************
 *   程序名：checkproc.cpp，此程序是守护程序：检查共享内存中进程的心跳，如果超时，则终止进程。      
*****************************************************************************************/
#include "_public.h"
using namespace idc;
...
int main(int argc,char *argv[])
{
    ...
    if (argc != 2)
    {
        ...
        printf("Example:/myproject/tools/bin/procctl 10 /myproject/tools/bin/checkproc /myproject/log/checkproc.log\n\n");
    ...
    for (int ii=0;ii<MAXNUMP;ii++)
    {
        ...
        // 一定要把进程的结构体备份出来，不能直接用共享内存中的值。因为如果使用共享内存的原值，下面发送完15的信号后\
            进程如果正常退出，共享内存会被清理。则89行kill（pid，0）；里的pid不是进程pid，是被清理过的内存为0；变成\
            kill(0,0)，会发给自己。循环5次后，在98行，kill(0,9);把自己杀死
