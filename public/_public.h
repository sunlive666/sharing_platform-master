...
bool sformat(string &str,const char* fmt, Args... args ) 
{
    ...
    snprintf(&str[0], len + 1, fmt, args... );           // linux平台第二个参数是len+1，windows平台是len。直接操作string中的内存，&str[0]为内存的首地址
    return true;
}
//结构化字符串，后面的变量要用c风格字符串
template< typename... Args >
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// 写文件的类。
//优化前：写文件步骤：1）打开文件--> 2)写入数据 --> 3)关闭文件
//但是文件操作没有锁机制，生成文件需要时间，
//如果进程A写文件的同时，进程B读取文件，那么可能导致进程B读取的文件是不完整的
//优化后：写文件步骤：1)打开临时文件--> 2)向临时文件写入数据--> 3)关闭临时文件--> 4)把临时文件名改为正式文件名
//demo36.cpp 写二进制文件 demo38.cpp
...
public:
    ...
    //可以指定什么是一行，根据业务需求，有时候多行是需要算一行输出的
    //例如</time>算借位标志
    //    123456
    //   dfdfdf
    // fdfdfdfdfdf</time> 这三行算一行
...
public:
    ...
    // bbackup：是否自动切换（备份），true-切换，false-不切换，在多进程的服务程序中，如果多个进程共用一个日志文件，bbackup必须为false。缺省为true。
    // benbuffer：是否启用文件缓冲机制，true-启用，false-不启用，如果启用缓冲区，那么写进日志文件中的内容不会立即写入文件，缺省是不启用(一般不启用）。
