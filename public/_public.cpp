...
char *deletelchr(char* str, const int cc)
{
    ...
    //memmove(str, p, strlen(p)+1);自己写的，是一样的
...
//函数里，tcpwrite()和之前的视频上学的socket发送不一样，demo1.cpp和demo2.cpp演示了，如果客户端一瞬间发送数据给服务端
//会出现TCP分包和粘包现象
...
//tcp的发送和接收：发送：把数据放到tcp的发送缓冲区。接收：从tcp接收缓冲区中取数据。
//分包和粘包：
//分包:
//     tcp报文的大小缺省是1460 字节，如果发送缓冲区中的数据超过1460字节，tcp将拆分成多个包发送，
//     如果接收方及时的从接收缓冲区中取走了数据，看上去像就接收到了多个报文
//粘包：
//     tcp接收到数据之后，有序放在接收缓冲区中，数据之间不存在分隔符的说法，
//     如果接收方没有及时的从缓冲区中取走数据看上去就象粘在了一起
//使用报文长度(四字节整数)+报文内容来区分报文，eg:Hello world,先接收11，在接收Hello world 
//如果使用原生的send和recv函数，将发生分包和粘包的现象
